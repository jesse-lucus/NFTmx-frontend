...
import OrderSolanaCard from "@/core/components/cards/OrderSolanaCard.vue";
...
watchEffect(async () => {
  if(walletAddress.value){
    ...
    const wallet = new PublicKey(walletAddress.value);
    const metas = await getParsedNftAccountsByOwner({
    ...
    {
      let metadata = await axios.get(metas[i].data.uri);
      console.log(metadata);
      const nft = {
        name: metas[i].data.name,
        symbol: metas[i].data.symbol,
