...
import sendLamport from "../../action/transaction";
...
// Upload image to Arweave
  console.log("imagedata",logo.value);
  if(logo.value) {
    const ext = '.' + logo.value[0].type.split('/')[1];
    console.log("ext", ext);
    const [link, imageLink] = await upload(name.value, symbol.value, ext, logoPreview.value);
...
<template>
  ...
  <body-container>
    ...
    <div>
      ...
      <setting-item-row class>
        <template v-slot:item>
          <div class="font-ibm-bold text-lg pt-3.5">Symbol</div>
        </template>
        <template v-slot:value>
          <div class="font-ibm text-sm text-tertiary-400 mt-3 md:mt-0">
            <nftmx-icon-input v-model="symbol" />
          </div>
        </template>
      </setting-item-row>
