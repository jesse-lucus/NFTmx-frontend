...
marketService.getUSDFromToken(TokenType.BNB).then((res) => {
  price.value =
    store.getters["market/etherFromWei"](props.order.tokenPrice) * res.data.USD;
});
...
const handleVote = () => {
  ...
  if (vote.value) {
    marketService
      .vote(props.order.tokenAddress, props.order.tokenId, store.state.user.id)
      .then((res) => {
        voteCount.value++;
      });
  } else {
  ...
  <div ref="card" class="w-full">
    ...
    >
      <div
        ...
        :style="{
          background: `url(${order.image_url})`,
